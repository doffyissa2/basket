...
export default function BottomNav({ active }: { active: 'home' | 'scan' | 'profile' }) {
  return (
    <nav
    ...
    >
      <LayoutGroup>
        <div className="flex items-end justify-around px-4 pb-safe" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 12px)', paddingTop: '8px' }}>
          {TABS.map((tab) => {
            ...
            return (
              <a key={tab.id} href={tab.href} className="flex flex-col items-center gap-1 py-1 relative min-w-[56px]">
                <motion.div
                  whileTap={{ scale: 0.85 }}
                  transition={{ type: 'spring', stiffness: 400, damping: 25 }}
