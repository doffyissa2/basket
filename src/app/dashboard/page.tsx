    ...
    <aside className="hidden md:flex flex-col w-64 flex-shrink-0 h-screen sticky top-0 bg-offwhite"
      ...
      {totalSavings > 0 && (
        <div className="mx-3 mb-3 rounded-2xl p-4" style={{ background: 'rgba(0,208,156,0.08)', border: '1px solid rgba(0,208,156,0.15)' }}>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-graphite/40 mb-1">Économies totales</p>
      ...
      <div className="px-3 pb-4 space-y-1" style={{ borderTop: '1px solid rgba(17,17,17,0.06)', paddingTop: 12 }}>
        <motion.a href="/notifications" whileHover={{ x: 2 }} whileTap={{ scale: 0.98 }}
          ...
          {unreadCount > 0 && (
            <span className="ml-auto w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center text-white" style={{ background: '#7ed957' }}>
    ...
    return (
      <div className="min-h-[100dvh] bg-paper flex items-center justify-center">
        <motion.div
          ...
          className="flex items-center gap-3 text-graphite/50"
        >
  ...
  return (
    <div className="min-h-[100dvh] bg-paper text-graphite flex">
      ...
      <div className="flex-1 overflow-y-auto pb-28 md:pb-0">
        ...
        {/* Mobile header */}
        <div className="md:hidden flex items-center justify-between px-5 pt-14 pb-5">
          <div>
            <p className="text-[10px] text-graphite/40 font-semibold uppercase tracking-widest mb-0.5">Tableau de bord</p>
            <h1 className="text-lg font-bold text-graphite">Bonjour, {greeting} 👋</h1>
          </div>
          <div className="flex items-center gap-2">
            <motion.a href="/notifications" whileTap={{ scale: 0.9 }} className="relative">
              <div className="w-9 h-9 rounded-full flex items-center justify-center glass">
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center text-white" style={{ background: '#7ed957' }}>
                    {unreadCount > 9 ? '9+' : unreadCount}
                  </span>
                )}
              </div>
            ...
            <motion.a href="/profile" whileTap={{ scale: 0.9 }}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center glass">
                <Settings className="w-4 h-4 text-graphite/50" />
              </div>
            </motion.a>
          </div>
        </div>
        ...
        {/* Desktop header */}
        <div className="hidden md:flex items-center justify-between px-8 py-6" style={{ borderBottom: '1px solid rgba(17,17,17,0.06)' }}>
          <div>
        ...
        <main className="px-5 md:px-8 py-5 md:py-6 max-w-3xl space-y-5">
          ...
          <motion.div
            ...
            className="relative rounded-3xl p-6 overflow-hidden bg-white"
          ...
          >
            ...
            <div className="relative">
              <p className="text-[10px] font-semibold text-graphite/40 uppercase tracking-widest mb-2">Économies totales</p>
              ...
              {totalSavings > 0 && (
                <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold"
                  style={{ background: 'rgba(0,208,156,0.1)', color: '#00D09C', border: '1px solid rgba(0,208,156,0.2)' }}>
          ...
          >
            {[
              ...
              { icon: History, label: 'Historique', href: '#', color: 'rgba(17,17,17,0.4)', bg: 'rgba(17,17,17,0.05)' },
            ].map((action) => (
              <motion.a
                key={action.label}
                href={action.href}
              ...
              >
                <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: action.bg }}>
                  <action.icon className="w-4 h-4" style={{ color: action.color }} />
                </div>
                <span className="text-[11px] font-semibold text-graphite/60">{action.label}</span>
              </motion.a>
            ))}
          </motion.div>
          ...
          {userPostcode && (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4" style={{ color: '#7ed957' }} />
                  <p className="text-sm font-bold text-graphite">Dans votre secteur ({userPostcode})</p>
                </div>
                <motion.a
                  href="/carte"
              ...
              {areaInsight ? (
                <p className="text-xs text-graphite/50 leading-relaxed">
                  En {userPostcode},{' '}
                  <span className="text-graphite font-semibold">{areaInsight.cheapestChain}</span>{' '}
                  est l'enseigne la moins chère pour votre panier habituel.
                </p>
              ) : (
                <p className="text-xs text-graphite/35 leading-relaxed">
          ...
          {/* Scan CTA — only shown if NO receipts yet */}
          <AnimatePresence>
            {(!hasReceipts) && (
              <motion.a
                href="/scan"
                ...
                className="block rounded-3xl p-6 relative overflow-hidden bg-white"
              ...
              >
                <div className="flex items-center gap-4">
                  <motion.div
                    ...
                    animate={{ boxShadow: ['0 0 0 0 rgba(17,17,17,0.2)', '0 0 0 14px rgba(17,17,17,0)', '0 0 0 0 rgba(17,17,17,0)'] }}
                    transition={{ duration: 2, repeat: Infinity }}
                  >
                    <Camera className="w-7 h-7 text-white" />
                  </motion.div>
                  <div className="flex-1">
                    <p className="font-bold text-base text-graphite">Scannez votre premier ticket</p>
                    <p className="text-sm text-graphite/50 mt-0.5">Découvrez combien vous pourriez économiser</p>
                  </div>
          ...
          <motion.a
            href="/scan"
            className="hidden md:block rounded-3xl p-5 relative overflow-hidden bg-white"
            style={{ border: '1px solid rgba(17,17,17,0.08)' }}
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: '#111111' }}>
                <Camera className="w-6 h-6 text-white" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-sm text-graphite">Scanner un ticket</p>
                <p className="text-xs text-graphite/40">Analyser et comparer les prix en quelques secondes</p>
              </div>
              <div className="flex items-center gap-1.5 text-xs font-semibold flex-shrink-0" style={{ color: '#7ed957' }}>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-graphite/35">{recentReceipts.length}</span>
            </div>

            {recentReceipts.length === 0 ? (
              <div className="glass rounded-2xl p-8 text-center">
                <div className="w-12 h-12 rounded-2xl glass flex items-center justify-center mx-auto mb-3">
                  <History className="w-6 h-6 text-graphite/30" />
                </div>
                <p className="text-sm text-graphite/60">Aucun ticket scanné</p>
                <p className="text-xs text-graphite/35 mt-1">Scannez votre premier ticket pour commencer</p>
              </div>
            ) : (
                ...
                {recentReceipts.slice(0, 5).map((receipt, i) => (
                  ...
                  >
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                      ...
                      <History className="w-4 h-4" style={{ color: '#7ed957' }} />
                    </div>
          ...
          >
            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(126,217,87,0.12)' }}>
              <Zap className="w-4 h-4" style={{ color: '#7ed957' }} />
            </div>
            <div>
              <p className="text-xs font-semibold text-graphite mb-0.5">Le saviez-vous ?</p>
              <p className="text-xs text-graphite/50 leading-relaxed">
