...
export default function BilanPage() {
  ...
  const downloadCard = () => {
    ...
    // Big savings number
    ctx.fillStyle = totalSavings > 0 ? '#00D09C' : '#FFFFFF'
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-28">
      ...
      <div className="flex items-center justify-between px-5 pt-14 pb-6">
        ...
        <h1 className="text-lg font-bold">Bilan de la semaine</h1>
        <div className="w-16" />
      </div>
      ...
      <main className="max-w-lg mx-auto px-5 space-y-4">
        ...
        >
          {/* Glow */}
          <div className="absolute top-0 right-0 w-56 h-56 rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #00D09C 0%, transparent 70%)', transform: 'translate(30%,-30%)' }} />

          <p className="text-xs text-[#6B7280] mb-3">{weekLabel}</p>

          {/* Big savings */}
          <p className="font-extrabold mb-1" style={{ fontSize: 56, lineHeight: 1, color: totalSavings > 0 ? '#00D09C' : '#FFFFFF', fontVariantNumeric: 'tabular-nums' }}>
            €{totalSavings.toFixed(2)}
          </p>
          <p className="text-sm text-[#6B7280] mb-5">économisés cette semaine</p>

          {/* Stats row */}
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex items-center gap-1.5">
              <Receipt className="w-4 h-4 text-[#E07A5F]" />
              <span className="text-sm font-semibold">{scanCount} ticket{scanCount !== 1 ? 's' : ''}</span>
            </div>
            {streak > 1 && (
              <div className="flex items-center gap-1.5">
                <Flame className="w-4 h-4 text-orange-400" />
                <span className="text-sm font-semibold">{streak} semaines</span>
              </div>
            )}
            {topStore && (
              <div className="flex items-center gap-1.5">
                <TrendingDown className="w-4 h-4 text-[#00D09C]" />
                <span className="text-sm font-semibold">{topStore}</span>
              </div>
            )}
          </div>

          {/* Branding */}
          <p className="absolute bottom-4 right-5 text-xs text-[#E07A5F] font-bold opacity-60">basket.fr</p>
        </motion.div>

        {/* No data state */}
        {scanCount === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8 glass rounded-2xl px-6">
            <p className="text-sm font-semibold text-white mb-1">Aucun ticket cette semaine</p>
            <p className="text-xs text-[#4B5563]">Scannez un ticket pour voir votre bilan</p>
          </motion.div>
        )}
        ...
        {bestDeal > 0 && (
          ...
          >
            ...
            <div>
              <p className="text-xs text-[#6B7280]">Meilleure économie</p>
              <p className="text-sm font-bold text-white">€{bestDeal.toFixed(2)} sur un ticket</p>
            </div>
          </motion.div>
        )}
        ...
        {weekReceipts.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass rounded-2xl overflow-hidden">
            <p className="text-xs font-semibold text-[#4B5563] uppercase tracking-wider px-4 py-3 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
              Tickets cette semaine
            </p>
            {weekReceipts.map((r, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3 border-b last:border-0" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                <div>
                  <p className="text-sm font-medium text-white">{r.store_name || 'Magasin'}</p>
                  <p className="text-xs text-[#4B5563]">{new Date(r.created_at).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'short' })}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold">{r.total_amount ? `${r.total_amount.toFixed(2)} €` : '—'}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-2 gap-3">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="h-12 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 glass"
          >
