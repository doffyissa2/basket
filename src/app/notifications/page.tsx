    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-28">
      ...
      <div className="flex items-center justify-between px-5 pt-14 pb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold">Alertes</h1>
          {unreadCount > 0 && (
            <span className="w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-white" style={{ background: '#E07A5F' }}>
              {unreadCount}
            </span>
          )}
        </div>
        {unreadCount > 0 ? (
          <button onClick={markAllRead} className="text-xs text-[#E07A5F] font-semibold">
            Tout lire
          </button>
        ) : <div className="w-16" />}
      </div>
        ...
        {notifications.length === 0 ? (
          ...
          >
            <div className="w-16 h-16 rounded-3xl glass flex items-center justify-center mx-auto mb-4">
              <Bell className="w-8 h-8 text-[#4B5563]" />
            </div>
            <p className="text-sm font-medium text-white mb-1">Aucune alerte</p>
            ...
            <AnimatePresence initial={false}>
              {notifications.map((notif, i) => (
                ...
                >
                  {/* Unread dot */}
                  {!notif.read && (
                    <div className="absolute top-4 right-4 w-2 h-2 rounded-full" style={{ background: '#E07A5F' }} />
                  )}

                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: notif.type === 'price_drop' ? 'rgba(0,208,156,0.15)' : 'rgba(224,122,95,0.15)' }}
                  >
                  ...
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <p className="text-xs text-[#6B7280] leading-relaxed">{notif.body}</p>
                    {notif.metadata?.new_price && notif.metadata?.old_price && (
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-xs line-through text-[#4B5563]">€{notif.metadata.old_price.toFixed(2)}</span>
                        <span className="text-xs font-bold" style={{ color: '#00D09C' }}>€{notif.metadata.new_price.toFixed(2)}</span>
                        {notif.metadata.store && (
                          <span className="text-xs text-[#4B5563]">chez {notif.metadata.store}</span>
                        )}
                      </div>
                    )}
                  ...
                  {notif.read && (
                    <Check className="w-4 h-4 text-[#4B5563] flex-shrink-0 mt-0.5" />
                  )}
                </motion.div>
              ))}
            </AnimatePresence>
